...
for i in range(len(F_sample)):
    for j in range(len(F_sample[i])):
        for k in range(len(F_sample[i][j])):
            if i+j+k % 3 == 0:
                F_sample[i][j][k] = 0
...
for epoch in range(num_epochs):
    for i in range(len(F_sample)):
        for j in range(len(F_sample[i])):
            inputs = F_sample[i]
            model = NMF(n_components=5, init='random', random_state=0)
            W = model.fit_transform(inputs)
            H = model.components_
            pre = np.matmul(W,H)
            pre = torch.tensor(pre).float().cuda()
            optimizer.zero_grad()  # zero the gradient buffer
            outputs = net((pre[j]))
            loss = criterion(outputs,F_train[i][j])
            loss.backward()
            optimizer.step()
        if i % 100 == 0:
            print(i,"/",len(F_train), " Loss : ", loss.data)
...
for i in range(len(F_test)):
    for j in range(len(F_test[i])):
        ...
        res += loss.data
    if i % 100 == 0 and i!=0:
        print(i,"/",len(F_train), " Loss : ", res / ((i+1)*j))
